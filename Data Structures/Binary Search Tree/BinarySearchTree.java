...
public class BinarySearchTree<T> extends AbstractSet<T>
{
    ...
    public void computeAverageDepth(BinarySearchTree<T> binarySearchTree){

        Map<Node<T>,Integer> hashMap=new HashMap<>();
        Set<Integer> distancesToRoot=new HashSet<>();

        Integer distanceToRoot=0;
        Integer overallSum=0;

        if(binarySearchTree.root==null) return ;
           ...
           // Node<T> root=getRoot();
            ...
            while(!queue.isEmpty()){
                ...
                if(tempNode.left!=null) {
                    ...
                    hashMap.put(tempNode.left,hashMap.get(tempNode)+1);
                    overallSum+=hashMap.get(tempNode.left);
                    System.out.println(tempNode.left.element);
                }
                if(tempNode.right!=null) {
                    queue.add(tempNode.right);
                    hashMap.put(tempNode.right,hashMap.get(tempNode)+1);
                    overallSum+=hashMap.get(tempNode.right);
                    System.out.println(tempNode.right.element);
                }
        ...
        while (it.hasNext()) {
            ...
            Node<T> currentNode=(Node<T>)pair.getKey();
            System.out.println(currentNode.element+ " = " + pair.getValue());
            it.remove(); // avoids a ConcurrentModificationException
        }

        System.out.println("Overall sum is "+overallSum+" number of nodes are "+size);
        System.out.println("Average depth is"+(double)overallSum/this.size);
    }
